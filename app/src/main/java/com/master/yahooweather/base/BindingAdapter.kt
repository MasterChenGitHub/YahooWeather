...
@BindingAdapter("app:setWeatherIconFromNet")
fun setWeatherIconFromNet(view: ImageView, iconPath: String?) {
    if (iconPath.isNullOrEmpty())
        return
    Picasso.get().cancelRequest(view)
//http://l.yimg.com/a/i/us/we/52/32.gif
    Picasso.get().load(iconPath).fit().centerCrop()
        .placeholder(R.drawable.launcher_foreground)
        .error(R.drawable.launcher_foreground)
        .into(view)
}
